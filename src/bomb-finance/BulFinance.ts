...
import {ContractName, TokenStat, AllocationTime, LPStat, Bank, PoolStats, DShareSwapperStat, PlanInfo, ResultInfo, DepositInfo} from './types';
...
export class BulFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.DGN = new ERC20(deployments.Dgn.address, provider, 'DGN');
    this.DSHARE = new ERC20(deployments.DShare.address, provider, 'DSHARE');
    this.DBOND = new ERC20(deployments.DBond.address, provider, 'DBOND');
    this.BNB = this.externalTokens['WBNB'];
    this.BTC = this.externalTokens['BTCB'];
    this.BASH = this.externalTokens['BASH'];
    this.XBUL = new ERC20(deployments.xDGN.address, provider, 'XBUL');
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.DGN, this.DSHARE, this.DBOND, ...Object.values(this.externalTokens)];
  ...
  async getBulStat(): Promise<TokenStat> {
    const {DgnRewardPool, DgnGenesisRewardPool} = this.contracts;
    const supply = await this.DGN.totalSupply();
    const bombRewardPoolSupply = await this.DGN.balanceOf(DgnGenesisRewardPool.address);
    const bombRewardPoolSupply2 = await this.DGN.balanceOf(DgnRewardPool.address);
    const bombCirculatingSupply = supply.sub(bombRewardPoolSupply).sub(bombRewardPoolSupply2);
    //  const priceInBNB = await this.getTokenPriceFromPancakeswap(this.DGN);
     //const priceInBNBstring = priceInBNB.toString();
     const priceInBTC = await this.getTokenPriceFromPancakeswapBTC(this.DGN);
    ...
    //  console.log('priceOfBulInDollars DGN', priceInDollars);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.DGN.decimal, 0),
      circulatingSupply: getDisplayBalance(bombCirculatingSupply, this.DGN.decimal, 0),
    };
  }
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('DGN') ? this.DGN : this.DSHARE;
    const isBul = name.startsWith('DGN');
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    const priceOfDBondInDollars = (Number(bombStat.priceInDollars) * modifier).toFixed(4);
    const supply = await this.DBOND.displayedTotalSupply();
    return {
      tokenInFtm: bondPriceInBNB,
      priceInDollars: priceOfDBondInDollars,
      totalSupply: supply,
  ...
  async getShareStat(): Promise<TokenStat> {
    const {DShareRewardPool} = this.contracts;

    const supply = await this.DSHARE.totalSupply();

    const priceInBNB = await this.getTokenPriceFromPancakeswap(this.DSHARE);
    const bombRewardPoolSupply = await this.DSHARE.balanceOf(DShareRewardPool.address);
  ...
  async getBulStatInEstimatedTWAP(): Promise<TokenStat> {
    const {Oracle, DgnRewardPool} = this.contracts;
    const expectedPrice = await Oracle.twap(this.DGN.address, ethers.utils.parseEther('100'));

    const supply = await this.DGN.totalSupply();
    const bombRewardPoolSupply = await this.DGN.balanceOf(DgnRewardPool.address);
  ...
  async getBulPriceInLastTWAP(): Promise<BigNumber> {
    const {Treasury} = this.contracts;
    return Treasury.getBulUpdatedPrice();
  }
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'DGN' ? await this.getBulStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'DGN') {
      if (!contractName.endsWith('DgnRewardPool')) {
        const rewardPerSecond = await poolContract.dgnPerSecond();
      ...
      if (Date.now() - startDateTime.getTime() > FOUR_DAYS) {
        return await poolContract.epochDgnPerSecond(1);
      }
      return await poolContract.epochDgnPerSecond(0);
    }
    const rewardPerSecond = await poolContract.tSharePerSecond();
    console.log('==========> debug2 ========>', earnTokenName,contractName, depositTokenName)
    if (!depositTokenName.startsWith('DSHARE')) {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WBNB') {
      ...
      if (tokenName === 'DGN-BNB-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.DGN, true);
      } else if (tokenName === 'DSHARE-BNB-LP') {
        
        tokenPrice = await this.getLPTokenPrice(token, this.DSHARE, false);
      } else if (tokenName === 'DSHARE-BNB-APELP') {
        tokenPrice = await this.getApeLPTokenPrice(token, this.DSHARE, false);
      } else if (tokenName === 'DGN-BTCB-APELP') {
        tokenPrice = await this.getApeLPTokenPrice(token, this.DGN, true);
      } else {
  ...
  async withdraw(): Promise<TransactionResponse> {
    const {BashStake} = this.contracts;
    return await BashStake.withdraw();
  }

  async getPlanInfo(plan: string | number): Promise<PlanInfo> {
    const {BashStake} = this.contracts;
    return BashStake.getPlanInfo(plan);
  }

  async getResult(plan: string | number, deposit: string | number): Promise<ResultInfo> {
    const {BashStake} = this.contracts;
  ...
  async getUserDepositInfo(address: string, index : string | number): Promise<DepositInfo> {
    const {BashStake} = this.contracts;
    return BashStake.getUserDepositInfo(address, index);
  }
  ...
  async getTotalValueLocked(): Promise<Number> {
    ...
    const boardroomtShareBalanceOf = await this.DSHARE.balanceOf(this.currentBoardroom().address);
    const bombStakeBalanceOf = await this.DGN.balanceOf(this.XBUL.address);
    const boardroomTVL = Number(getDisplayBalance(boardroomtShareBalanceOf, this.DSHARE.decimal)) * Number(DSHAREPrice);
    
    const bombTVL = Number(getDisplayBalance(bombStakeBalanceOf, this.DGN.decimal)) * Number(BULPrice);
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'DGN') {
        return await pool.pendingBUL(poolId, account);
      } else {
  ...
  async getTokenPriceFromPancakeswapBULUSD(): Promise<string> {
    ...
    const {WBNB} = this.config.externalTokens;
     ...
     const wbnb = new Token(56, WBNB[0], WBNB[1]);
    const btcb = new Token(56, this.BTC.address, this.BTC.decimal, 'BTCB', 'BTCB');
    const token = new Token(56, this.DGN.address, this.DGN.decimal, this.DGN.symbol);
    try {
      ...
      console.error(`Failed to fetch token price of ${this.DGN.symbol}: ${err}`);
    }
  }
  ...
  async getBoardroomAPR() {
    ...
    const boardroomtShareBalanceOf = await this.DSHARE.balanceOf(Boardroom.address);
    const boardroomTVL = Number(getDisplayBalance(boardroomtShareBalanceOf, this.DSHARE.decimal)) * Number(DSHAREPrice);
  ...
  async getBulStakeAPR() {
    ...
    //Mgod formula
    const amountOfRewardsPerDay = epochRewardsPerShare * Number(BULPrice) * 4;
    const xBulBulBalanceOf = await this.DGN.balanceOf(this.XBUL.address);
  ...
  async stakeShareToBoardroom(amount: string): Promise<TransactionResponse> {
    if (this.isOldBoardroomMember()) {
      throw new Error("you're using old boardroom. please withdraw and deposit the DSHARE again.");
    }
    const Boardroom = this.currentBoardroom();
  ...
  async stakeToBul(amount: string): Promise<TransactionResponse> {
    const Xbul = this.contracts.xDGN;
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === BNB_TICKER) {
      ...
      const token = tokenName === BUL_TICKER ? this.DGN : this.DSHARE;
  ...
  async swapDBondToDShare(bbondAmount: BigNumber): Promise<TransactionResponse> {
    const {DShareSwapper} = this.contracts;
    return await DShareSwapper.swapDBondToDShare(bbondAmount);
  }
  async estimateAmountOfDShare(bbondAmount: string): Promise<string> {
    const {DShareSwapper} = this.contracts;
    try {
      const estimateBN = await DShareSwapper.estimateAmountOfDShare(parseUnits(bbondAmount, 18));
      return getDisplayBalance(estimateBN, 18, 6);
    } catch (err) {
      console.error(`Failed to fetch estimate dshare amount: ${err}`);
    }
  }

  async getDShareSwapperStat(address: string): Promise<DShareSwapperStat> {
    const {DShareSwapper} = this.contracts;
    const bulshareBalanceBN = await DShareSwapper.getDShareBalance();
    const bbondBalanceBN = await DShareSwapper.getDBondBalance(address);
    // const bombPriceBN = await DShareSwapper.getBulPrice();
    // const bulsharePriceBN = await DShareSwapper.getDSharePrice();
    const rateDSharePerBulBN = await DShareSwapper.getDShareAmountPerBul();
    ...
    return {
      ...
      rateDSharePerBul: rateDSharePerBulBN.toString(),
    };
  }
}
