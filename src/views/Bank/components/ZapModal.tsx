...
const ZapModal: React.FC<ZapProps> = ({onConfirm, onDismiss, tokenName = '', decimals = 18}) => {
  ...
  const bulshareBalance = useTokenBalance(bombFinance.DSHARE);
  ...
  const tShareFtmLpStats = useLpStats('DSHARE-BNB-LP');
  const bombLPStats = useMemo(() => (bombFtmLpStats ? bombFtmLpStats : null), [bombFtmLpStats]);
  const bulshareLPStats = useMemo(() => (tShareFtmLpStats ? tShareFtmLpStats : null), [tShareFtmLpStats]);
  const ftmAmountPerLP = tokenName.startsWith(BUL_TICKER) ? bombLPStats?.ftmAmount : bulshareLPStats?.ftmAmount;
  /**
  ...
  return (
    <Modal>
      ...
      <Select onChange={handleChangeAsset} style={{color: '#2c2560'}} labelId="label" id="select" value={zappingToken}>
        ...
        <StyledMenuItem value={DSHARE_TICKER}>DSHARE</StyledMenuItem>
        ...
        {/* <StyledMenuItem value={BUL_TICKER}>DGN</StyledMenuItem> */}
      </Select>
      <TokenInput
